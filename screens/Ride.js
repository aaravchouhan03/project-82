...
export default class RideScreen extends Component {
    ...
    handleTransaction = async () => {
        ...
        if (!transactionType) {
          ...
          // Check user eligibility for starting a ride
          ...
          // Check user eligibility for ending a ride
    ...
    checkUserEligibilityForStartRide = async (userId) => {
        ...
        if (userRef.docs.length === 0) {
          // If there are no matching documents, set bikeId to empty and isUserEligible to false
          this.setState({
            bikeId: "",
          });
          ...
          userRef.docs.map((doc) => {
            if (!doc.data().bike_assigned) {
              // If the user doesn't have a bike assigned, set isUserEligible to true
              isUserEligible = true;
            } else {
              // If the user has a bike assigned, set isUserEligible to false and show an alert
              isUserEligible = false;
    ...
    checkUserEligibilityForEndRide = async (bikeId, userId) => {
        ...
        var isUserEligible = false; // Initialize isUserEligible as false
      
        transactionRef.docs.map((doc) => {
          var lastBikeTransaction = doc.data();
          // Check if 'lastBikeTransaction.user_id' equals 'userId'.
